...
import { USER_STORAGE_KEY, authProvider } from './authProvider';
...
const dataProviderWithCustomMethod = {
  ...
  unarchivePayment: async (payment: Payment) => {
    ...
    const { data: payments } = await baseDataProvider.getList<Payment>(
      'payments',
      {
  ...
  [
    {
      ...
      beforeDelete: async params => {
        ...
        const [companies, contacts, contactNotes, payments] = await Promise.all(
          [
